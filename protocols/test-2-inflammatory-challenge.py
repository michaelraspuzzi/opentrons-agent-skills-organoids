...
Deck Layout:
    ┌─────┬─────┬─────┬─────┐
    │ A1  │ A2  │ A3  │ A4  │  ← Tips, Tips, (empty), Staging
    ├─────┼─────┼─────┼─────┤
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p8_200.flow_rate.aspirate = 100   # µL/s (slower for cells)
    p8_200.flow_rate.dispense = 150   # µL/s
    ...
    def add_treatment(source, wells, volume, description):
        ...
        for well_name in wells:
            ...
            p8_200.aspirate(volume, source.bottom(z=2))
            # Dispense above existing media (z=5mm from bottom)
            p8_200.dispense(volume, cell_plate.wells_by_name()[well_name].bottom(z=5))
            # Touch tip to remove droplets
            p8_200.touch_tip()
        
        # Blow out any remaining liquid
        p8_200.blow_out(source.top())
        p8_200.drop_tip()
    ...
    for well_name in all_treatment_wells:
        p8_200.pick_up_tip()
        p8_200.aspirate(fluo4_vol, fluo4.bottom(z=2))
        p8_200.dispense(fluo4_vol, cell_plate.wells_by_name()[well_name].bottom(z=5))
        p8_200.touch_tip()
        p8_200.blow_out(cell_plate.wells_by_name()[well_name].top())
        p8_200.drop_tip()
