...
FIXES APPLIED:
1. Changed from 8-channel to single-channel pipette (plate layout incompatible with 8-channel)
2. Removed duplicate apiLevel from metadata (keep only in requirements)
3. Added trash bin definition for tip disposal
"""
...
metadata = {
    ...
    'author': 'Generated by Claude - FIXED VERSION',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # CHANGED: Using single-channel instead of 8-channel
    # Reason: Plate layout has mixed treatments in same columns
    p1000 = protocol.load_instrument(
        'flex_1channel_1000',  # Changed from flex_8channel_1000
        'right',
        tip_racks=[tips_1000_1, tips_1000_2]
    )

    # Set conservative flow rates for gentle handling of organoids
    # (per liquid-handling-tips.md best practices)
    p1000.flow_rate.aspirate = 100   # µL/s (slower for cells)
    p1000.flow_rate.dispense = 150   # µL/s
    ...
    media = reservoir['A1']       # Vehicle control - 10 mL
    tnf_10 = reservoir['A2']      # TNF-α 10 ng/mL - 3 mL
    tnf_50 = reservoir['A3']      # TNF-α 50 ng/mL - 3 mL
    il1b_10 = reservoir['A4']     # IL-1β 10 ng/mL - 3 mL
    il1b_50 = reservoir['A5']     # IL-1β 50 ng/mL - 3 mL
    cond_media = reservoir['A6']  # Conditioned media - 3 mL
    fluo4 = reservoir['A12']      # Fluo-4 AM 2 µM - 8 mL

    # =========================================================================
    # WELL DEFINITIONS
    # =========================================================================

    # Define well groups based on plate layout
    ...
    # Blank wells for background (no Fluo-4 needed)
    ...
    def add_treatment(source, wells, volume, description):
        ...
        Includes touch_tip and blow_out per liquid-handling-tips.md best practices.
        """
        protocol.comment(f'Adding {description}')
        p1000.pick_up_tip()

        for well_name in wells:
            # Aspirate from reservoir (bottom z=2mm to avoid dead volume)
            p1000.aspirate(volume, source.bottom(z=2))
            # Dispense above existing media (z=5mm from bottom)
            p1000.dispense(volume, cell_plate.wells_by_name()[well_name].bottom(z=5))
            # Touch tip to remove droplets
            p1000.touch_tip()
    ...
    protocol.comment('Total wells treated: 72')
    ...
    # Use new tip for each transfer to prevent contamination
...
### Estimated Tip Usage
- Treatment addition: ~6 tips (one per treatment group)
- Fluo-4 addition: ~72 tips (one per well)
- Total: ~78 tips = 1 rack minimum (have 2 for safety)
